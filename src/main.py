    ...
    for step in range(num_steps):
        ...
        if not distributed or (distributed and rank == 0):
            if logger:
                ...
                logger.info(f"Step {step+1}/{num_steps} - Average accuracy so far: {current_avg_acc:.2f}%")
                
                if len(forgetting_measures) > 0:
                    current_avg_fgt = np.mean(forgetting_measures)
                    logger.info(f"Step {step+1}/{num_steps} - Average forgetting so far: {current_avg_fgt:.4f}")
