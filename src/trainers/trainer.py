...
from src.utils.metrics import accuracy, forgetting
...
class ContinualTrainer:
    ...
    def _limit_batches(self, data_loader: DataLoader, num_batches: int) -> DataLoader:
        ...
        limited_loader = DataLoader(
            ...
            pin_memory=data_loader.pin_memory if hasattr(data_loader, 'pin_memory') else False,
            sampler=None,  # We're using a subset, so we don't need a sampler
            drop_last=data_loader.drop_last if hasattr(data_loader, 'drop_last') else False
        )
    ...
    def train_step(
        ...
        print(f"{debug_prefix}=== Training Step {step+1}/{self.continual_config['num_steps']} ===")
        ...
        if debug_enabled:
            ...
            if self.debug_config.get("fast_dev_run", False):
                ...
                if train_limit < 1.0:
                    train_loader = self._limit_batches(train_loader, int(len(train_loader) * train_limit))
                    if not self.distributed or (self.distributed and self.local_rank == 0):
                        print(f"[DEBUG] Limited training to {train_limit * 100}% of batches ({len(train_loader)} batches)")
                
                if val_limit < 1.0:
                    test_loader = self._limit_batches(test_loader, int(len(test_loader) * val_limit))
                    if not self.distributed or (self.distributed and self.local_rank == 0):
                        print(f"[DEBUG] Limited validation to {val_limit * 100}% of batches ({len(test_loader)} batches)")
    ...
    def _train_epoch(
        ...
        should_show_pbar = not self.distributed or (self.distributed and self.local_rank == 0)
        ...
        for inputs, targets in train_iter:
            ...
            if self.continual_config["strategy"] == "ewc" and step > 0 and self.ewc_data is not None:
    ...
    def _evaluate(self, test_loader: DataLoader) -> Tuple[float, float]:
        ...
        with torch.no_grad():
            test_iter = tqdm(test_loader, desc=desc) if should_show_pbar else test_loader
    ...
    def _log_metrics(
        ...
        debug_enabled = self.debug_config.get("enabled", False)
        log_every_n_steps = self.debug_config.get("log_every_n_steps", 1) if debug_enabled else 1
        ...
        # Print metrics
        debug_prefix = "[DEBUG] " if debug_enabled else ""
        print(f"{debug_prefix}Epoch {epoch+1}: "
    ...
    def _compute_ewc_data(self, train_loader: DataLoader):
        ...
        params = {n: p.clone().detach() for n, p in model_to_use.named_parameters() if p.requires_grad}
        ...
        # Only show progress bar on main process if distributed
        should_show_pbar = not self.distributed or (self.distributed and self.local_rank == 0)
        data_iter = tqdm(train_loader, desc="Computing EWC data") if should_show_pbar else train_loader
    ...
    def _compute_ewc_loss(self) -> torch.Tensor:
        ...
        for n, p in model_to_use.named_parameters():
            if n in self.ewc_data["fisher"] and p.requires_grad:
                ...
                loss += (self.ewc_data["fisher"][n] * (p - self.ewc_data["params"][n]).pow(2)).sum()
